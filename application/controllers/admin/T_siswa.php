...
class T_siswa extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'admin/t_siswa/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'admin/t_siswa/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'admin/t_siswa/index.html';
            $config['first_url'] = base_url() . 'admin/t_siswa/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->T_siswa_model->total_rows($q);
        $t_siswa = $this->T_siswa_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            't_siswa_data' => $t_siswa,
            'q' => $q,
        ...
        $this->load->view('admin/t_siswa/t_siswa_list', $data);
        $this->load->view('admin/footer');
    }

    public function read($id) 
    {
        $row = $this->T_siswa_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'pekerjaan_wali' => $row->pekerjaan_wali,
            ...
            $this->load->view('admin/header');
            $this->load->view('admin/t_siswa/t_siswa_read', $data);
            $this->load->view('admin/footer');
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('admin/t_siswa'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('admin/t_siswa/create_action'),
        ...
        $this->load->view('admin/header');
        $this->load->view('admin/t_siswa/t_siswa_form', $data);
        $this->load->view('admin/footer');
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('admin/t_siswa/update_action'),
    ...
    public function update_action() 
    {
		...
		'pekerjaan_wali' => $this->input->post('pekerjaan_wali',TRUE),
            ...
            $this->T_siswa_model->update($this->input->post('id_siswa', TRUE), $data);
    ...
    public function word()
    {
        ...
        $data = array(
            't_siswa_data' => $this->T_siswa_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('admin/t_siswa/t_siswa_doc',$data);
    }
