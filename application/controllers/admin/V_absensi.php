...
class V_absensi extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('V_absensi_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'admin/v_absensi/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'admin/v_absensi/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'admin/v_absensi/index.html';
            $config['first_url'] = base_url() . 'admin/v_absensi/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->V_absensi_model->total_rows($q);
        $v_absensi = $this->V_absensi_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'v_absensi_data' => $v_absensi,
            'q' => $q,
        ...
        $this->load->view('admin/v_absensi/v_absensi_list', $data);
        $this->load->view('admin/footer');
    }

    public function read($id) 
    {
        $row = $this->V_absensi_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'tahun' => $row->tahun,
            ...
            $this->load->view('admin/header');
            $this->load->view('admin/v_absensi/v_absensi_read', $data);
            $this->load->view('admin/footer');
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('admin/v_absensi'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('admin/v_absensi/create_action'),
        ...
        $this->load->view('admin/header');
        $this->load->view('admin/v_absensi/v_absensi_form', $data);
        $this->load->view('admin/footer');
    }
    ...
    public function create_action() 
    {
		...
		'tahun' => $this->input->post('tahun',TRUE),
            ...
            $this->V_absensi_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('admin/v_absensi/update_action'),
    ...
    public function update_action() 
    {
		...
		'tahun' => $this->input->post('tahun',TRUE),
            ...
            $this->V_absensi_model->update($this->input->post('', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->V_absensi_model->delete($id);
	...
	foreach ($this->V_absensi_model->get_all() as $data) {
            $kolombody = 0;

            //ubah xlsWriteLabel menjadi xlsWriteNumber untuk kolom numeric
            xlsWriteNumber($tablebody, $kolombody++, $nourut);
	    xlsWriteLabel($tablebody, $kolombody++, $data->id_absensi);
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=v_absensi.doc");

        $data = array(
            'v_absensi_data' => $this->V_absensi_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('admin/v_absensi/v_absensi_doc',$data);
    }

}

/* End of file V_absensi.php */
/* Location: ./application/controllers/V_absensi.php */
